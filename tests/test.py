...
def do_shell(cmd, check=True, env=None, timeout=None, capture_output=True):
    ...
    if pipe_opt is not None:
        # read stdout/stderr up to a maximum number of bytes (10MB)
        # fun fact: as the comment for this, Copilot suggested this link: https://stackoverflow.com/a/375427
        ...
        def read_fds():
            has_data = False
            rlist, _, _ = select.select([p.stdout, p.stderr], [], [], 1)
            for (lst, fd) in [(stdout, p.stdout), (stderr, p.stderr)]:
                if fd in rlist:
                    bytes_to_read = MAX_OUTPUT_BYTES - sum(map(len, lst))
                    if bytes_to_read > 0:
                        lst.append(fd.read())
                        has_data = True
            return has_data

        # wait for the build to finish, collecting up to MAX_OUTPUT_BYTES of each output stream
        while p.poll() is None and (time.time() - start_time < timeout if timeout is not None else True) and ((sum(map(len, stdout)) < MAX_OUTPUT_BYTES) or (sum(map(len, stderr)) < MAX_OUTPUT_BYTES)):
            read_data = read_fds()
            if not read_data:
                time.sleep(0.1)

        read_fds()
        timeout_expired = (time.time() - start_time >= timeout if timeout is not None else False)
        p.kill()
...
def get_test_files_root():
    if override_test_dir:
        return os.path.abspath(override_test_dir)
    else:
...
def check_compiler_output(spec, return_code, compiler_output):
    ...
    if 'should_error' in spec and spec['should_error']:
        if return_code == 0:
            print_failure(f'FAILED TO REJECT ILLEGAL FILE -- {name}')
            print_bench_error('EE')
            return False
        else:
            print_success(f'CORRECTLY REJECTED -- {name}')
            return True
    if return_code != 0:
        print_failure(f'FAILED TO COMPILE -- {name}')
    ...
    if 'should_match' in spec:
        ...
        if compiler_output.strip() != expected.strip():
            print_failure(f'WRONG OUTPUT -- {name}')
...
def do_hyperfine_benchmark(name, run_dir, exec_fname):
    ...
    print_grade(f'bench {name} {mean_time} {num_outliers}')
    return result['times']
...
def do_derby_benchmark(run_dir, exec_fname):
    if True or not autograder:
        ...
        print_grade(f'bench derby {mean_time_ms}')
        return
...
def do_run(spec, run_dir, asm):
    ...
    if not skip_compile:
        ...
        if 'lib_dir' in spec:
            ...
            if platform == "darwin":
                cmd += ['-L', spec['lib_dir'], '-l6035_macOS', '-lpthread']
            else:
        ...
        if return_code != 0:
            print_failure(f'FAILED TO ASSEMBLE -- {name}')
    ...
    if timed_out:
        print_failure(f'RUNTIME TIME LIMIT EXCEEDED -- {name}')
    ...
    if result is not None and result and benchmark:
        if 'derby' in spec and spec['derby']:
            ...
            do_hyperfine_benchmark(name, run_dir, exec_fname)
...
def check_run_output(spec, return_code, run_output):
    ...
    if 'should_run_error' in spec and spec['should_run_error']:
        if return_code == 0:
            print_failure(f'FAILED TO THROW AN ERROR -- {name}')
            print_bench_error('WE')
            return False
        else:
            print_success(f'CORRECTLY THREW AN ERROR -- {name}')
            return True
    if return_code != 0:
        print_failure(f'RUNTIME ERROR -- {name}')
...
def do_test(spec):
    ...
    try:
        ...
        timed_out = True
    if timed_out:
        print_failure(f'TIME LIMIT EXCEEDED DURING COMPILATION -- {name}')
...
def do_tests(specs):
    ...
    for spec in specs:
        skip_test = (
            ...
            not fnmatch.fnmatch(spec['name'], test_filter) and
            not fnmatch.fnmatch(spec['name'].split('.')[0], test_filter)
        )
        if not skip_test:
            q.put(spec)
    if num_jobs > 1:
...
def parse_args():
    ...
    parser.add_argument('testsets', nargs='+', type=str, choices=testset_names, help='test set to run')
    ...
    parser.add_argument('-f', '--filter', dest='filter', type=str, default=None, help='only run tests matching this pattern (glob)')
    ...
    parser.add_argument('-O', '--opt', dest='opt', type=str, default=None, help='perform only these optimizations')
    parser.add_argument('-a', '--args', dest='args', type=str, default=None, help='additional arguments when running compiler')
    parser.add_argument('-D', '--working-directory', dest='run_dir', type=str, default=None, help='location to write and run executable (default: temp dir)')
    ...
    parser.add_argument('--test-dir', dest='test_dir', type=str, default=None, help='directory containing test files')
    parser.add_argument('--project-root', dest='project_root', type=str, default=None, help='directory containing project files')
    parser.add_argument('--no-color', dest='no_color', action='store_const', const=True, default=False, help='disable color output')
...
if __name__ == '__main__':
    ...
    if args.project_root is not None:
        project_root = os.path.abspath(os.path.realpath(args.project_root))
    else:
        project_root = os.path.abspath(os.path.realpath('.'))


    if args.test_dir:
        override_test_dir = os.path.abspath(os.path.realpath(args.test_dir))
    else:
        override_test_dir = os.path.dirname(os.path.abspath(os.path.realpath(__file__)))
